...
├── api/                   # Endpoint API (Scan processing, Geolocation checkin, Stats)
├── assets/                # Aset Statis (CSS Tailwind/Custom, JS, SVG Icons & Logos)
├── auth/                  # Modul Autentikasi (Login, Register School, Google SSO, Logout)
...
└── index.php              # Router & Entry Point Aplikasi
```
...
1. **Clone Repositori:**
   ```bash
   git clone https://github.com/abdrohman968/HadirTadz.git
   ```
2. Letakkan di folder `www` (Laragon) atau `htdocs` (XAMPP).
3. **Setup Database:**
