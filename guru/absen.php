<?php
$page_title = 'Absen Masuk & Pulang (GPS)';
...
<main class="flex-1 overflow-y-auto bg-slate-50 p-4 sm:p-6 lg:p-8">
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <!-- Page Header -->
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Presensi Mandiri Guru (GPS & Kamera)</h1>
            <p class="text-xs sm:text-sm text-slate-500">Lakukan absensi masuk atau pulang menggunakan validasi lokasi GPS sekolah dan foto selfie.</p>
        </div>

        <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
            
            <!-- GPS Radius Status Box -->
            <div id="gps-status-card" class="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 id="gps-title" class="text-xs font-bold text-slate-700">Mendeteksi Lokasi GPS Anda...</h4>
                        <p id="gps-desc" class="text-[11px] text-slate-500">Mohon izinkan akses lokasi (GPS) pada browser Anda.</p>
                    </div>
                </div>
            ...
            <div class="space-y-2">
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider">Kamera Selfie (Verifikasi Wajah)</label>
                <div class="relative w-full aspect-video sm:aspect-[4/3] max-w-md mx-auto rounded-3xl overflow-hidden bg-black border-2 border-slate-200 shadow-inner flex items-center justify-center">
                    ...
                    <img id="selfie-preview" class="hidden w-full h-full object-cover transform -scale-x-100" />
                </div>
            </div>
        ...
        if (navigator.geolocation) {
            navigator.geolocation.watchPosition(pos => {
                ...
                if (isWithin) {
                    ...
                    title.textContent = "Di Luar Jangkauan Sekolah";
                    desc.textContent = `Jarak Anda ${dist}m melebihi batas maksimal ${radiusLimit}m`;
